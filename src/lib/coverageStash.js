...
// they added a document, so the counts should move. It persists on this browser so a
// reader can fetch a document or complete checkout without losing the job in progress.
//
...
/** Historical name retained for callers; resuming no longer consumes the saved job. */
export function takeCoverageStash() {
  return readCoverageStash();
}
